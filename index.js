...
for (i = 0; i < artists.length; i++) {
  if (artists[i].name.includes("Dough")) {
    console.log("Before change: ", artists[i].name);
    artists[i].name = "Vincent Van Gogh";
    console.log("After change: ", artists[i].name);
  }
}
...
console.log(getArtistByIndex(artists, 50));
...
text("Task 6")
function addArtist(newArtistObj) {
  const newArtist = {
    id: artists.length+1,
    ...newArtistObj
  }
...
const jeff = {
  ...
  genre: "Web Developer",
  nationality: "German",
  bio: "Loves art, but is not an artist.",
  paintings: 0,
  wikipedia: null
}
const kyle = {
  ...
  genre: "Web Developer",
  nationality: "Mixed",
  bio: "Baskeball and KH are his arts.",
  paintings: 1,
  wikipedia: null
}
console.log(addArtist(jeff))
console.log(addArtist(kyle))
